...
public class SearchService {
    ...
    public Map<String, Object> searchTutors(SearchRequest request) {
        String sortKey = request.getSortBy() != null ? request.getSortBy() : "rating";
        ...
        switch (sortKey) {
            case "price_asc":
                sort = Sort.by(Sort.Direction.ASC, "monthlyRate");
                break;
            case "price_desc":
                sort = Sort.by(Sort.Direction.DESC, "monthlyRate");
                break;
            case "experience":
                sort = Sort.by(Sort.Direction.DESC, "experienceYears");
                break;
            case "name":
                // name not directly mapped; fall back to rating
            case "rating":
            default:
        ...
        if (request.getCity() != null && !request.getCity().isEmpty()) {
            cityId = cityRepo.findByName(request.getCity()).map(City::getId).orElse(null);
        }

        LocalTime reqStart = parseTime(request.getAvailabilityStart());
        LocalTime reqEnd = parseTime(request.getAvailabilityEnd());

        Page<TutorDetails> tutors = tutorDetailsRepo.searchTutors(
                cityId,
                request.getMinPrice(),
                request.getMaxPrice(),
        ...
        for (TutorDetails tutor : tutors.getContent()) {
        ...
        resp.put("page", tutors.getNumber());
        resp.put("size", tutors.getSize());
        resp.put("totalElements", tutors.getTotalElements());
        resp.put("totalPages", tutors.getTotalPages());
        return resp;
    }
