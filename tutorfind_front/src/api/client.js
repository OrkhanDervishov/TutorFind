...
export const api = {
  ...
  // Returns array of tutors; if backend wraps in {content, ...} unwrap it
  searchTutors: async (params) => {
    const res = await request(`/api/tutors${toQueryString(params)}`);
    return Array.isArray(res) ? res : (res && res.content ? res.content : []);
  },
