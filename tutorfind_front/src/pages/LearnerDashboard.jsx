                ...
                <article key={session.id} className="request-card">
                  <div>
                    <div className="request-card__header">
                      <h4>{prettyName(session.tutorName, "Tutor", session.tutorId)}</h4>
                      <span className={statusClass(session.status)}>
                        {STATUS_LABELS[session.status] || session.status}
                      </span>
                    </div>
                    <p className="hint">Booking #{session.id}</p>
            ...
            <label className="field">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  const b = sessions.find((s) => String(s.id) === bookingId);
                  setReviewForm((prev) => ({
                    ...prev,
                    bookingId,
                    tutorId: b ? b.tutorId : ""
              ...
              >
                ...
                {sessions.map((s) => (
                  <option key={s.id} value={s.id}>
                    {`Booking #${s.id} — ${prettyName(s.tutorName, "Tutor", s.tutorId)} — ${
                      s.subject || ""
                    } — ${s.slot || ""}`}
                  </option>
                ))}
