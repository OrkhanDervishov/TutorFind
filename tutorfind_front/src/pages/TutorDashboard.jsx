...
const prettyName = (name, role, id) => {
  if (!name) return role ? `${role} #${id || ""}` : `#${id || ""}`;
  if (id) return `${name} (${role} #${id})`;
  return name;
};
...
const TutorDashboard = () => {
  ...
  const onSelectFeedbackBooking = (bookingId) => {
    setSelectedFeedbackBookingId(bookingId);
    const b = acceptedBookings.find((x) => String(x.id) === bookingId);
    if (b) {
      setFeedbackForm((prev) => ({
        ...prev,
        bookingId: b.id,
        learnerId: b.learnerId,
        subjectId: b.subjectId || prev.subjectId
      }));
    } else {
      setFeedbackForm((prev) => ({
        ...prev,
                ...
                <article key={req.id} className="request-card">
                  <div>
                    <div className="request-card__header">
                      <h4>{prettyName(req.learnerName, "Learner", req.learnerId)}</h4>
                      <span
                        className={`badge ${
                          req.status === "ACCEPTED"
                            ? "badge--green"
                    ...
                    <p className="hint">Booking #{req.id}</p>
        ...
        <div className="card mt-4">
          ...
          <form className="stacked-form" onSubmit={submitFeedback}>
            <label className="field">
              <span>Select booking (Accepted)</span>
              <select
                value={selectedFeedbackBookingId}
                onChange={(e) => onSelectFeedbackBooking(e.target.value)}
              >
                <option value="">Choose a booking</option>
                {acceptedBookings.map((b) => (
                  <option key={b.id} value={b.id}>
                    {`Booking #${b.id} — ${prettyName(
                      b.learnerName,
                      "Learner",
                      b.learnerId
                    )} — ${b.subject || ""} — ${b.slot || ""}`}
            ...
            <div className="two-col">
              ...
              <label className="field">
                ...
                <input
                  ...
                  placeholder="Prefilled if booking has subject"
                />
              </label>
            </div>
            <label className="field">
            ...
            <button type="submit" className="btn btn--primary" disabled={!feedbackForm.learnerId}>
              Submit Feedback
            </button>
            {!feedbackForm.learnerId && (
              <p className="hint">Select a booking to prefill learner and booking IDs.</p>
            )}
          </form>
